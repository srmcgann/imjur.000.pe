...
export default {
  ...
  data(){
    return {
      state: {
        footerMsg: '<b>IMJUR</b><br>A FREE DIGITAL ASSET<br>HOSTING SERVICE - ©'+(new Date()).getFullYear() + `<br>contact: <a href="mailto:whitehotrobot@gmail.com">whitehotrobot@gmail.com</a>`,
        links: [],
        userLinks: [],
      ...
      switch(this.state.mode){
        ...
        case 'default':
          ...
          if(this.state.curPage){
            ...
            history.pushState(null,null,this.URLbase) + search
          }
        break
        case 'track':
          window.location.href = this.URLbase + '/track/' + this.decToAlpha(this.state.curTrack) + '/' +(this.state.curPage + 2) + search
